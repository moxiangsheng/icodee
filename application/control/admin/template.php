...
class admin_templatecontrol extends base {
    ...
    function admin_templatecontrol(& $get, & $post) {
      ...
      function ondefault($message='', $type='correctmsg') {
       ...
       chmod("view/".$dir, 0777); 
       		  file_put_contents("view/".$dir."/".$dir_file.".html",stripslashes(htmlspecialchars_decode($this->post['tpl_content'])));
       		  
       $tpl_content=file_get_contents("view/".$dir."/".$dir_file.".html");
       	}else{
       		 $dir= $this->get[2];
       $dir_file=$this->get[3];
       
       $tpl_content=htmlspecialchars(file_get_contents("view/".$dir."/".$dir_file.".html"));
